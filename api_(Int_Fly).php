...
// --- BATCH 6: ACCORPAMENTO / FATTURAZIONE DIFFERITA ---
		...
		// --- RECUPERO DOCUMENTI DA ACCORPARE ---
        case 'get_documenti_da_accorpare':
            $tipoDoc = (int)($_GET['Tipo'] ?? 0);
            $idCliente = (int)($_GET['IDCliente'] ?? 0);
            
            // IL FIX È QUI: Usiamo SELECT * per estrarre TUTTI i campi della fattura, 
            // così DocumentDetail avrà tutto ciò che gli serve per funzionare!
        ...
        case 'esegui_accorpamento':
            ...
            try {
                ...
                // 4. Copia le righe (FattureCorpo) dai vecchi documenti al nuovo
                $sqlCopyRows = "INSERT INTO fatturecorpo (IDFatt, Codart, Descrzione, Quant, ImpUnit, Iva, Magazz, ttiva, sconto, unmis, impon, imposta, ordine)
                                SELECT ?, Codart, Descrzione, Quant, ImpUnit, Iva, Magazz, ttiva, sconto, unmis, impon, imposta, ordine 
                                FROM fatturecorpo WHERE IDFatt IN ($placeholders)";
                $paramsCopy = array_merge([$newDocId], $docIds);
                $pdo->prepare($sqlCopyRows)->execute($paramsCopy);

                // 5. FIX 2: Aggiorna i vecchi documenti usando il campo corretto 'accorpa = -1'
                $sqlUpdateOld = "UPDATE fatture SET accorpa = -1 WHERE ID IN ($placeholders)";
                $pdo->prepare($sqlUpdateOld)->execute($docIds);

                $pdo->commit();
                // Restituiamo l'ID del nuovo documento per poterlo aprire in React!
